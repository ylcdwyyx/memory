...
- `memory_targets.toml`：目标文件列表，可设定额外的抬头或结尾。
...
1. 根据自身需求编辑 `src/memory_sync/sync.py` 顶部的 `MEMORY_SOURCE` 常量；若更偏好维护独立文件，可将 `memory_targets.toml` 的 `source` 改成对应路径。
2. 修改 `memory_targets.toml`，将 `path` 指向各个工具的用户记忆文件；可选字段 `header`、`footer` 用于附加前后缀。
